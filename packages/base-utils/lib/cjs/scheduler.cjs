...
//#region packages/base-utils/src/scheduler.ts
/**
* 一个队列调度器实现
*/
var Scheduler = class {
	...
	constructor(options = {}) {
		...
		if (this.options.concurrency < 1) throw new Error("并发数必须大于0");
		for (;;) {
	...
	optionInit(options) {
		for (const [key, item] of Object.entries(options)) this.options[key] = item;
	}
	pickShortestQueue() {
		let shortestQueue = this.queueList[0];
		for (const queue of this.queueList) if (queue.map.size < shortestQueue.map.size) shortestQueue = queue;
	...
	add(task) {
		...
		this.pickShortestQueue().map.set(Symbol(), taskRecord);
		if (this.options.checkRunAfterAdd && this.pause) this.run();
		return result;
	}
	async run() {
		for (const record of this.queueList) this.runQueue(record);
	}
	async runQueue(queueRecord) {
		...
		if (queueRecord.map.size !== 0) await this.runQueue(queueRecord);
